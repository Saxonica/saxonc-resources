...
def test_saxon_edition(saxonproc):
    assert saxonproc.edition == '@@edition@@'.upper()
...
def test_make_atomic_value1(saxonproc):
    value1 = saxonproc.make_atomic_value("string", "hello world")
    assert value1 is not None
    assert "hello world" in str(value1)

    value2 = saxonproc.make_atomic_value("integer", "2")
    assert value2 is not None
    assert value2.integer_value == 2

    value3 = saxonproc.make_atomic_value("date", "2018-08-08")

    assert value3 is not None
    assert "2018" in str(value3)


def test_make_atomic_value_none(saxonproc):

    value1 = saxonproc.make_atomic_value("", "hello world")
    assert value1 is None

    value2 = saxonproc.make_atomic_value(None, "hello world")
    assert value2 is None

    value3 = saxonproc.make_atomic_value("string", None)
    assert value3 is None

    value4 = saxonproc.make_atomic_value("stringx", "hello world")
    assert value4 is None

    try:
        value5 = saxonproc.make_atomic_value("integer", "hello")
        print(value5)
    except Exception as err:
        print("Error: ", err)
        assert True
...
def test_Xslt_from_file2(saxonproc, data_dir):
    xsltproc = saxonproc.new_xslt30_processor()
    xmlFile = os.path.join(data_dir, "cat.xml")
    result = xsltproc.transform_to_string(source_file=os.path.join(data_dir, "cat.xml"),
                                          stylesheet_file=os.path.join(data_dir, "test.xsl"))
    assert result is not None
    assert 'text3' in result
...
def test_Xslt_from_file_error(saxonproc, data_dir):
    xsltproc = saxonproc.new_xslt30_processor()
    try:
        result = xsltproc.transform_to_value(source_file=os.path.join(data_dir, "cat.xml"),
                                             stylesheet_file=os.path.join(data_dir, "test-error.xsl"))
...
def test_xslt_parameter(saxonproc, data_dir):
    input_ = saxonproc.parse_xml(
        xml_text="<out><person>text1</person><person>text2</person><person>text3</person></out>")
...
def testUTF8(saxonproc):
    ...
    trans = saxonproc.new_xslt30_processor()
    executable = trans.compile_stylesheet(
        stylesheet_text="<xsl:stylesheet version='2.0' xmlns:xsl='http://www.w3.org/1999/XSL/Transform'><xsl:template match='e'>UTF8-تيست: <xsl:value-of select='.'/></xsl:template></xsl:stylesheet>",
        encoding="UTF-8")
...
def testContextNotRoot(saxonproc):
    ...
    trans = saxonproc.new_xslt30_processor()
    executable = trans.compile_stylesheet(
        stylesheet_text="<xsl:stylesheet version='2.0' xmlns:xsl='http://www.w3.org/1999/XSL/Transform'><xsl:variable name='x' select='.'/><xsl:template match='/'>errorA</xsl:template><xsl:template match='e'>[<xsl:value-of select='name($x)'/>]</xsl:template></xsl:stylesheet>")
    assert node is not None
    assert isinstance(node, PyXdmNode)
    assert len(node.children) > 0
    eNode = node.children[0].children[0]
...
def test_equals(saxonproc):
    node = saxonproc.parse_xml(xml_text="<doc><e>text</e></doc>")
    assert node is not None
    assert isinstance(node, PyXdmNode)
    assert len(node.children) > 0
    eNode1 = node.children[0]
    eNode2 = node.children[0]
    assert eNode1.equals(eNode2)
...
def testNodeAxis(saxonproc, data_dir):
    node_ = saxonproc.parse_xml(
        xml_text="<out xmlns:my='http://www.example.com/ns/various' xmlns:f='http://www.example.com/ns/various1' xmlns='http://www.example.com'><person>text1</person><person>text2</person></out>")
...
def testXquery_40_functions():
    ...
    result = xquery_processor.run_query_to_value(
        query_text='xquery version "4.0"; parse-html("<p>This is paragraph 1.<p>This is paragraph 2.")', lang='4.0')
...
def testXquery_40_functions_error():
    try:
        ...
        xquery_processor = proc.new_xquery_processor()
        result = xquery_processor.run_query_to_value(
            query_text='xquery version "4.0"; parse-html("<p>This is paragraph 1.<p>This is paragraph 2.")', lang='3.0')
        assert False
    except Exception:
        assert True
...
def testContext2NotRootNamedTemplate(saxonproc):
    ...
    executable = trans.compile_stylesheet(
        stylesheet_text="<xsl:stylesheet version='2.0' xmlns:xsl='http://www.w3.org/1999/XSL/Transform'><xsl:variable name='x' select='.'/><xsl:template match='/'>errorA</xsl:template><xsl:template name='main'>[<xsl:value-of select='$x'/>]</xsl:template></xsl:stylesheet>")
...
def testXdmDestination1(saxonproc):
    trans = saxonproc.new_xslt30_processor()
    executable = trans.compile_stylesheet(
        stylesheet_text="<xsl:stylesheet version='2.0' xmlns:xsl='http://www.w3.org/1999/XSL/Transform'><xsl:template name='go'><a/></xsl:template></xsl:stylesheet>")
...
def testCallFunctionArgConversion(saxonproc):
    ...
    executable = trans.compile_stylesheet(stylesheet_text=source)
    v = executable.call_function_returning_value("{http://localhost/}add",
                                                 [saxonproc.make_integer_value(2), saxonproc.make_integer_value(3)])
    assert isinstance(v.head, PyXdmItem)
    assert v.head.is_atomic
...
def testParseJson(saxonproc):
    ...
    try:
        ...
        print(parsed_json1)
        assert True
    except saxonc.PySaxonApiError as e:
        print(e.message)
        assert False
    except Exception as e:
        print(e)
...
def testParseJsonFunction(saxonproc):
    ...
    parse_json_fn = saxonc.PyXdmFunctionItem.get_system_function(saxonproc,
...
def testCallTemplateNoParamsRaw(saxonproc):
    trans = saxonproc.new_xslt30_processor()
    executable = trans.compile_stylesheet(
        stylesheet_text="<xsl:stylesheet version='2.0' xmlns:xsl='http://www.w3.org/1999/XSL/Transform'><xsl:template name='xsl:initial-template'><xsl:sequence select='42'/></xsl:template></xsl:stylesheet>")
    executable.set_result_as_raw_value(True)
...
def testxQuery_encoding1(saxonproc, data_dir):
    ...
    result = query_proc.run_query_to_value(input_xdm_item=node, query_text="/out/salary/text() = '3000 €'",
                                           encoding="UTF-8")
...
def test_add_packages(data_dir):
    ...
    result = xsl.transform_to_string(source_file=os.path.join(data_dir, "package-00.xml"),
                                     stylesheet_file=os.path.join(data_dir, "package-019.xsl"))
...
def testMapOperations(saxonproc):
    mymap = {saxonproc.make_string_value("a"): saxonproc.make_integer_value(1),
             saxonproc.make_string_value("b"): saxonproc.make_integer_value(2),
...
def testMapOperationsWithStrings(saxonproc):
    mymap = {'a': "2", "b": saxonproc.make_integer_value(2)}
    ...
    assert map.get('a').head.get_string_value() is "2"
...
def testMapValues(saxonproc):
    mymap = {"a": saxonproc.make_integer_value(1), "b": saxonproc.make_integer_value(2),
             "c": saxonproc.make_integer_value(5)}
    xdmdict = create_xdm_dict(saxonproc, mymap)
    map = saxonproc.make_map(xdmdict)
    assert map is not None
    assert map.map_size == 3
